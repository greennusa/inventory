...
<?php
    ...
    $x = $p->toBase()->merge($l)->sortBy(function($user, $key)
    {
        return $user->pemakaian->diterima;
    });
    ...
    <body onload="window.print()">
        ...
        <div class="container">
            ...
            <center>
                ...
                <h5>LAPORAN BON PENGAMBILAN SPARE PART CHAINSAW KARYAWAN CAMP / DLL</h5>
            ...
            @if( $nama != $detail->pemakaian->diterima)
            <?php $nama = $detail->pemakaian->diterima; 
            $total_semua = 0;
			
            ?>
            <table class="table table-bordered table-hover">
                <b style="font-size: 14px;">Nama : {{ $detail->pemakaian->unit->operator }} ( {{ $detail->pemakaian->unit->kode }} )</b>
                <thead>
            ...
            @foreach( $x as $detail)
            @if(date('m',strtotime(@$detail->pemakaian->tanggal)) == $from && date('Y',strtotime(@$detail->pemakaian->tanggal)) == $tahun && $detail->pemakaian->diterima == $nama)
                    <tr>
                        ...
                        <?php  $d = DetailPemakaianBarang::where('id',@$detail->id)->where('barang_id',@$detail->barang->id)->whereHas('pemakaian',function ($q) use ($from,$tahun) {

                                $q->whereMonth('tanggal',$from)->whereYear('tanggal',$tahun);

                            })->get();

                                $dl = DetailPemakaianBarangLama::where('id',@$detail->id)->where('barang_id',@$detail->barang->id)->whereHas('pemakaian',function ($q) use ($from,$tahun) {
