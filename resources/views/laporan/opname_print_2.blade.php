<?php   
$kategori = \App\Kategori::find($_GET['kategori']);
...
<body onload="window.print()">
    ...
    <div class="container">
        ...
        <br>
        <center>
            <h5>LAPORAN STOK SPARE PART KATEGORI {{ $kategori->nama }}</h5>
        ...
        <?php 
            ...
            /*$tables = Camp::whereHas('detail_bbk', function($q) use($from){
				$q->whereHas('bbk', function($query) use($from){
					$query->whereMonth('tanggal', $from);
				});
			})->get()->toBase()->merge(CampLama::whereMonth('tanggal',$from)->get()); */
        ...
        <table class="table table-bordered table-hover">
                        ...
                        @foreach($tables as $data)
                            @if(@$data->barang->kategori->id == $_GET['kategori'])
                            <tr>
                                <td><?php echo $no ?></td>
                                ...
                                <td>Rp.<?php echo number_format((!isset($data->harga) ? $data->detail_bbk->detail_bbm->detail_pemesanan->detail_permintaan->harga : $data->harga)) ?></td>
                                ...
                                <td>{{ $pemakaian }}</td>
                                <td>{{ @$data->stok }}</td>
                                <td>Rp.{{ number_format(@$data->stok*(!isset($data->harga) ? $data->detail_bbk->harga : $data->harga)) }}</td>
